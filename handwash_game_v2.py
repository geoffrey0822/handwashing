...
missed_transitions=['','Missing step 1:\n Palms\nPlease follow the tutorial to correct your gesture','Missing step 2:\nBetween Fingers\nPlease follow the tutorial to correct your gesture','Missing step 3:\nBack of Hands\nPlease follow the tutorial to correct your gesture','Missing step 4:\nBack of Fingers\nPlease follow the tutorial to correct your gesture','Missing step 5:\nFinger Tips\nPlease follow the tutorial to correct your gesture','Missing step 6:\nThumbs\nPlease follow the tutorial to correct your gesture','Missing step 7:\nWrists\nPlease follow the tutorial to correct your gesture','']
...
class ResultContainer(QtGui.QWidget):
    ...
    def retrain(self):
        global gui3,pmode,cool_down_start,cool_down_time,uiUpdated,game_thread,threadStarted,missed
        missed=True
        gui3.nextPoseDummy()
        gui3.showFullScreen()
        pmode=2
        gui3.changeTransition()
...
class Game3GUI(QtGui.QWidget):
    def __init__(self,parent=None):
        ...
        self.lblHeading=QtGui.QLabel('Please follow the instruction below',self)
    ...
    def checkTransition(self):
        global transitions,missed_transitions,missed
        
        train_currentPose=self.todo_poses[0]
        if not missed:
            self.lblTransition.setText(transitions[train_currentPose])
        else:
            self.lblTransition.setText(missed_transitions[train_currentPose])          
        
    def clickNextPose(self):
        global current_pose_duration,current_pose_duration_limit,current_pose_startend,mainMenu,cool_down_start,cool_down_time,transitions,missed_transitions,missed
        current_pose_duration=0
        current_pose_startend=[0,0]
        if len(self.todo_poses)==1:
            pmode=-1
    ...
    def reinit(self):
        global current_pose_duration,current_pose_duration_limit,current_pose_startend,all_poses,passed_poses,train_currentPose,all_pose_durations,train_duration,transitions,missed_transitions,missed
        self.todo_poses=[]
    ...
    def nextPoseDummy(self):
        global current_pose_duration,current_pose_duration_limit,current_pose_startend,all_poses,passed_poses,train_currentPose,uiUpdated,infos,transitions,cool_down_start,cool_down_time,missed_transitions,missed
        train_currentPose=self.todo_poses[0]
        current_pose_duration=0
        current_pose_startend=[0,0]
        self.lblInfo.setText(infos[train_currentPose])
                                    ...
                                    if uiUpdated:
                                        if cool_down_time>0:
                                            ...
                                            if isManualNext:
                                                ...
                                                if class_idx>0 and class_idx<8:
                                                    if current_pose_duration<current_pose_duration_limit:
                                                        ltime=time.time()
                                                        if current_pose_startend[0]==0:
                                                            current_pose_startend[0]=ltime
